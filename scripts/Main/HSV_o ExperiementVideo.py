...
cap = cv2.VideoCapture("Balls01.mkv")
...
output = cv2.VideoWriter("output1.mp4", fourcc, fps, (frame_width, frame_height))
...
while True:
    ...
    # blue color
    low_blue = np.array([101, 107, 108])
    high_blue = np.array([179, 255, 255])
    blue_mask = cv2.inRange(hsv_frame, low_blue, high_blue)
    # red color
    low_red = np.array([99, 85, 159])
    high_red = np.array([179, 255, 255])
    red_mask = cv2.inRange(hsv_frame, low_red, high_red)
    # green color
    low_green = np.array([43, 67, 124])
    high_green = np.array([161, 255, 255])
